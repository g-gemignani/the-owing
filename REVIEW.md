...
| 8 | `assets/art/bg_treasure.png`, `bg_victory.png`, `bg_defeat.png` | ~~Visibly a flatter, unrendered art tier than the other 20 backdrops. Victory is the end-of-run screen.~~ **FIXED (D293)**, and it was six files rather than three: measuring ink density across all 27 backdrops put `bg_event`, `bg_rest` and `bg_shop` in the same band, so the whole of Tier 5c was re-rolled as one batch. Ink went from 0.58–7.38% to 9.62–12.65% against the dungeons' 3.9–11.4%, and `bg_victory`'s blown doorway from 5.797% of the frame over 0.90 to 0.000%. (Written as `bg_chest.png`, which has never existed — the chest screen draws `bg_treasure.png`. Corrected in D196 so the row names a file somebody can act on.) |
