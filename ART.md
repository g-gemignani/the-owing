...
| **Kenney 16×16 pixel art** | 41 enemy sprites, 5 zone tiles, 1 card sheet | Off-style, and semantically arbitrary — *the 41 enemy sprites are gone (D89); the zone tiles and the card sheet remain, and both are now fallbacks nothing reaches. The last live user was the isometric crawl, which named neither a scene nor a zone to `UI.screen()` and so fell through to the tiling pattern on the screen with the most turns in it — fixed in D267, which gives it the dungeon's own backdrop.* |
...
**Its MATERIALS are half painted now, and that half is a shopping-list item (D267).** The
floor and rock surfaces come off `tools/gen_iso_art.gd`, which samples each terrain's ramp
from the backdrops that use it — so the generated set agrees with the paintings on colour
and on nothing else. Since `GROUND_UV_TILES` and `WALL_UV_TILES` stopped crushing a whole
256px repeat into one 116×58 diamond, that gap is visible, and painted materials installed
over the generated ones by `tools/install_iso_material.gd` are what closes it.

| role | state |
|---|---|
| `floor_stone` `floor_earth` `floor_moss` `floor_sand` | painted |
| `rock_stone` `rock_moss` | painted |
| `floor` `rock` (generic fallbacks) | painted, from the stone pair |
| **`rock_earth` `rock_sand`** | **still generated — the two left to draw** |

The Warrens is where the gap shows: a painted floor under flat walls. Ask for a wall the
way D267 did — head-on, four courses across, the terrain's dark colours named as WORDS as
well as hexes, flat lighting, nothing pure black or white — then install with
`--luma=0.42`, which is what the wall tints were tuned against.
