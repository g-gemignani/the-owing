...
### D166 — Rules on one screen, progress on the screens that own it

Reported: *"In How the Owing Works we are mixing info about how to play the game and
info on the progress of the game, which is save specific."* Correct, and the mixing was
D133's doing — it moved the Builds tracker into the glossary because Builds is the one
meta screen with nothing to press, and "somewhere you read" seemed like the category.

**Reading vs doing is the wrong axis; true-for-everyone vs true-for-this-save is the
right one.** Everything else on that screen is a rule — the escrow, the death penalty,
what Block does, at this game's numbers. Builds counted what one player holds and named
what they must still go and get. Nothing marked the join, so a reader could not tell
which half of the screen was the game. `builds_screen.gd`'s own header had already
written the distinction down while arguing the other way: *"the glossary states rules
that are true for every save, and every line here is a fact about this one."*

So: **Builds is a standalone screen again, reached from the Collection** — the screen
that holds the cards it counts, with the fraction on the button because a count on a
menu entry is the entry's reason to be pressed. Not the world hub: the hub is three
doors and a way out, and this is a question about a collection. The rules screen keeps
one line naming where the tracking lives, because *where* a thing is tracked is itself
true of every save.

`tests/test_content.gd` now fails if `glossary.gd` mentions `MetaState` or `GameState`
in code. It is a crude rule and it is the exact rule that was broken: the screen may
read `Balance`, which is the same for everyone, and may not read the save.

## The dungeon panels, and what the data would not let them be

Asked for: *"an info panel for every dungeon that lists the cards that we are missing or
not."* Measured before drawing anything, because `card_pool_for()` is the zone's themed
pool PLUS the dungeon's exclusives, and the split is lopsided:
...
`ZoneView` lists every dungeon of a region on ONE screen, so a full pool under each
would have printed the same nineteen cards three times, the three lists differing by two
rows. That is the duplication D133 spent a screen removing. **What is per-dungeon is the
count and the exclusives; what is per-region is the pool** — so each dungeon carries
"Cards found here: you hold 9 of 21" and its own exclusives drawn out, and the shared
pool is drawn once, at the bottom, under the dungeons rather than above them, because
this screen's job is choosing a door and the region grid is five rows tall.

A locked dungeon gets both. What is behind a door you cannot open yet is the reason to
go and open it, and this screen already names the boss of one.

## One slot widget, and the bonus

The slot — effect symbol, name, dimmed if unowned — was `builds_screen._slot`. It is
`UI.card_slot` now, used by both screens, because a dim name has to mean the same thing
in both places and two copies is how it stops meaning it. The three measured numbers
(column width, symbol size, dim) stay with the callers: each was derived against a
particular screen, and a default here would be one screen's measurement imposed on the
next. The region grid keeps the builds screen's four columns at 290; the exclusives
FLOW at 200, because one to three slots on a 290 grid put the second one 300px from the
first with nothing in between.

The asked-for bonus — *"hover over them also shows the card"* — is hover for the full
reading (generated by the one function that generates card text, so it cannot drift from
the card face) and **click for the card itself**, through the `UI.inspect_card` overlay
that combat, the collection and the deck builder already use. The tooltip says so and the
cursor changes, because an affordance nobody is told about is not one.

## Two things the layout got wrong first, both visible only in a capture

* `card_slot` set `SIZE_EXPAND_FILL` on its row, inherited from the grid it was born in.
  In a `GridContainer` that is invisible — every cell has the same minimum width — and in
  an `HFlowContainer` it makes each slot claim an equal share of the row, so two
  exclusives under a dungeon sat 500px apart with the second one in the middle of the
  backdrop. It is `SIZE_FILL`: the slot is as wide as it was asked to be, and packing is
  the container's business.
* The harness hands every capture all hundred cards, which is right for the screens that
  list what you hold and is the least informative possible state for a want-list — every
  slot lit, no dim one anywhere, so the half of the widget that does the work never
  appears. `tools/screenshots.gd` grew a `partial` flag (two cards in five removed,
  deterministically) and a `meta` flag (out of a run, since every row entered a dungeon
  and so photographed the Collection in its mid-run mode, which is the one without the
  Builds door on it). Flags combine with `+`. Builds had no row at all, having been
  reachable only inside the glossary — D123's blind spot, again: an absent row looks
...
### D174 — The reward asked the player to choose between three cards and told them nothing about any of them

Asked for: *"when choosing a card after winning an encounter I would like to know if any of
them is a card that I am missing in my collection, or how many copies I have and how many I
need of it for levelling it up, and what the level up is about."*

Every one of those is a fact the game already held and did not put on the screen. The reward
row priced the RUN cost of taking a card and nothing else — *"taking one makes your deck N
cards: you would see any given card every X turns instead of Y"* — which is the correct half
to state and is the half that expires when the run does. The permanent half is why the
collection exists: copies of one card buy that card's levels (D81's whole point, *"cards
earned is the wrong number; copies of ONE card is the right one"*), and the screen where
copies are handed out was the one screen that never mentioned them.

**Each offer is a column now: the card, and two lines under it.**

    NEW — no copy in your collection            Owned x3 — Lv1 of 100
    4 copies + 20g → Lv2: dmg 6→7               1 more copy + 20g → Lv2: dmg 6→7  (this one)

    Owned x9 — Lv2 of 100                       Owned x9 — Lv2 of 2
    Lv3 for 51g now: dmg 7→8                    Lv2 is its cap — spare copies buy nothing

Three numbers decide which of those it is, and the third is the one that was easy to get
wrong:

* `count` — copies in the permanent collection.
* `risk` — copies taken EARLIER in this run, which are not in the collection yet and never
  will be if the boss wins (D20). Counted and named separately (`· 1 copy taken this run`):
  folding them into the total would promise a level that dying erases.
* **the keep-one rule.** `MetaState.fuse` spends `fuse_copy_cost` copies and `can_fuse`
  refuses to spend the last one, so the copies you must OWN to buy a level is the cost PLUS
  ONE. Quote the bare cost and the reward screen promises a level at four copies that the
  Collection screen then refuses at four — after the player went and earned the fourth.

`tests/RewardNoteTest.tscn` does not check that arithmetic against a formula, because the
formula is the thing under test: it reads the number out of the note's own text, adds that
many copies, and requires `MetaState.can_fuse` to say yes — and requires one fewer to say no.

**The offer was quoted at level 1, which was a lie about the card being offered.** A reward
joins the run deck at the level the collection already holds that card at
(`GameState.earn_card`) and the catalogue `.tres` is the level-1 one,
so a Bash fused to Lv4 was offered as "Deals 8" and dealt 14 the moment it was taken. That is
D50's drift — *card faces show the current numbers, not the level-1 text* — surviving on the
one surface where the player is choosing between cards ON those numbers. `_roll_rewards`
stamps the collection's level on each offer, so the face, the level glow (D132) and the deck
now agree.

**Two wrappers answer with a wider number than the card has, and both were caught by writing
this note.**

* `MetaState.max_level(id)` is the RARITY's level track. `CardData.level_cap()` is where THIS
  card's numbers stop improving, and for a card whose whole effect is a status or a draw the
  second is far lower — measured, **25 of the 100 cards**, worst first:

      read_ahead     track 100   real  2      double_down    track 15   real 2
      see_it_coming  track  40   real  2      ram            track 40   real 2
      cold_read      track 100   real  5      set_stone      track  5   real 3
      focus          track 100   real  5      kick           track 15   real 3
      stumble        track 100   real  5      abyssal_gift   track 15   real 8

  "Lv1 of 100" over a card that is finished at Lv2 tells the player to go and earn copies for
  98 levels that do not exist. The note quotes `mini(max_level, level_cap)`.
* `MetaState.fuse_gold_cost(id)` returns the flat `FUSE_BASE_GOLD` for a card that is not in
  the collection yet — right for a wrapper with no entry to read, wrong for a screen offering
  the card. An unowned uncommon was quoted at 20g and priced at 52 the moment it was owned.
  The note prices through `Balance.fuse_gold_cost(card.rarity, level)`, which is the same call
  for an owned card.

**What was deliberately NOT changed, and it is a real defect left standing.** `MetaState.max_level`
still answers with the rarity track, so `collection.gd` still prints "Lv1/100" beside Read
Ahead and `fusable_levels` still counts levels above the effective cap as affordable. The one-line
fix is to make `max_level` return the card's own cap — but that changes what fusion ALLOWS
(`at_max_level` starts saying yes, `can_fuse` starts saying no) and not merely what a screen
says, and repricing what a copy can buy is a fusion decision that wants its own measurement of
the copy economy, not a side effect of a commit about the reward screen. So the reward note is
honest today and the Collection screen is not, which reads worse than leaving both wrong and is
the point: two screens disagreeing is visible, and both of them quoting the same wide number
is what hid it on every screen that prints a level.

**Where it lives.** `UI.collection_standing()` returns the two lines, the hover and the ink;
`UI.collection_line()` draws it. Split that way because the same sentence goes into three
places that cannot be allowed to disagree — the label under the card, the card's own tooltip,
and the `UI.inspect_card` overlay a right-click opens — and because the shop sells cards and
asks the identical question in a different rectangle. `card_button` grew a `note` parameter that
rides the route `card_slot`'s already did (D166).

**The note cost 60px and the panel had 4 to give.** `reward_box` is an anchored VBox at
0.30–0.86, which is 403px at the shipped 720. Card 246 + note 60 + the dilution line 23 +
Skip 50 + separations = **399**. The margin was four pixels, and it had been eleven before
the note existed — so the Skip button already grazed the HP bar and now sat on top of it and
on the "Encounter cleared. +8 gold" line at the bottom left.

Nothing clipped and nothing errored, because **a Control's size is never less than its
content**: the box quietly grew out of its own rectangle and drew the overflow over whatever
was under it. That is also why the first version of the fit assertion passed against a note
three lines too long — it compared the content against `reward_box.size.y`, which is the
content. It compares against the ANCHOR rect now.

The panel starts at 0.14, which gives the same 399px 518 to sit in. That the top of the
combat screen is empty at reward time is not luck — `_win()` clears the hand and the enemy
plates before it builds the row.

**And the reward pick is now photographed.** `tools/screenshots.gd` grew a `reward` pose and
a `CombatReward` row; the collision above is plainly visible in the first capture and
