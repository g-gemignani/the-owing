...
breaking it. The *what* — file-by-file detail and the full decision log D1–D142 — is
...
.github/     ci.yml — suite, then export-readiness, then (main only) a rolling
             release under the fixed tag `latest`. Godot cross-exports Windows
             and macOS from one Ubuntu runner, so all three desktop binaries
             come out of one job and no secret is involved. The tag never moves
             off that name: the README's download links are only stable URLs
             because of it (D142). actions/setup-godot/ is the shared
             cache-and-install step the three jobs share
DESIGN.md    the full reasoning, decision by decision (D1–D142)
