...
breaking it. The *what* — file-by-file detail and the full decision log D1–D154 — is
...
- **A harness that captures one state certifies one state.** `tools/IsoArtCheck.tscn` draws
  ...
  pixels rather than against the arguments.**
...
DESIGN.md    the full reasoning, decision by decision (D1–D154)
