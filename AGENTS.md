...
breaking it. The *what* — file-by-file detail and the full decision log D1–D145 — is
...
.github/     ci.yml — suite, then export-readiness, then (main only) three
             build-* jobs and one release job. Desktop and Android come off one
             Ubuntu runner; iOS needs macOS and is the ONLY job carrying
             continue-on-error, because Apple moves things and a gameplay fix
             must not sit unreleased for it (D144). No secret is involved: the
             Android key is a per-build throwaway and the .ipa is unsigned. The
...
DESIGN.md    the full reasoning, decision by decision (D1–D145)
