...
const NO_EXIT := {
	...
	# The stake is spent and the floor is generated by the time this screen opens (D253), so a way
	# out of it is a way to hold a run open with no power in it. Same rule as Encounter: a screen
	# that owes an answer does not offer a door, and all three choices are free.
	"PowerPick": "the run has already begun; every choice on it is free and one must be taken",
}
