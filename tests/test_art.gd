...
## A generator's measurements, or {} if there are none. The audio generators write one of
## these beside every set they ship, and it is the only thing in the repository that knows
## what the OGGs actually measure — the game cannot decode an OGG back to samples.
...
func _init() -> void:
	...
	# --- the sound has to be one set, and it has to happen somewhere (D173) ---------
	#
	# Both generators measure what they write and refuse to ship a file outside its band,
	# and none of that helps the game: the gates live in Python that nobody runs on a
	# gameplay branch, and what ships is the OGGs plus the `measurements.json` beside them.
	# So the game asks its own questions of that file. Three of them, each a failure mode
	# that is silent in play:
	#
	#   * a hand-made file dropped in beside the generated ones, which the measurements
	#     then do not describe (this is how the old three-pack set came about),
	#   * an effect with no tail, which is the D173 defect — a dry sound did not happen
	#     anywhere, and a set of them is a game happening on a desk instead of in a dungeon,
	#   * two sample rates, which is D150's actual bug and was never checked in the engine.
	for pair in [[A.DIR, "the effects"], [A.MUSIC_DIR, "the score"]]:
		if not FileAccess.file_exists(String(pair[0]) + "measurements.json"):
			fails += 1
			print("FAIL %s ship with no measurements.json — nothing measured them" % pair[1])
	var sfx_m: Dictionary = _read_json(A.DIR + "measurements.json")
	var music_m: Dictionary = _read_json(A.MUSIC_DIR + "measurements.json")
	var rates := {}
	for event in A.SOUNDS:
		var stem := String(A.SOUNDS[event])
		if not sfx_m.has(stem):
			fails += 1
			print("FAIL sound '%s' is not in measurements.json — it was not generated by %s" % [
				stem, "tools/gen_sfx.py"])
			continue
		var sm: Dictionary = sfx_m[stem]
		rates[int(sm.get("rate", 0))] = true
		# 80 ms is under every family floor the generator states, so this is not a second
		# opinion about the design — it is the line between "in a room" and "on a desk".
		# The old set had `ui_click` at 35 ms and `attack` at 65 ms.
		if float(sm.get("tail_ms", 0.0)) < 80.0:
			fails += 1
			print("FAIL sound '%s' has a %.0f ms tail — that is a dry sound" % [
				stem, float(sm.get("tail_ms", 0.0))])
		# And the counterweight: a sound in a big room that is late is worse than a dry one.
		if float(sm.get("onset_ms", 0.0)) > 140.0:
			fails += 1
			print("FAIL sound '%s' takes %.0f ms to be audible" % [
				stem, float(sm.get("onset_ms", 0.0))])
	for score in A.SCORES:
		var mstem := String(A.SCORES[score])
		if not music_m.has(mstem):
			fails += 1
			print("FAIL score '%s' is not in measurements.json" % mstem)
			continue
		rates[int((music_m[mstem] as Dictionary).get("rate", 0))] = true
	if rates.size() > 1:
		fails += 1
		print("FAIL the audio runs at %d different sample rates (%s) — that is not one set" % [
			rates.size(), str(rates.keys())])

	# Every place the player can be has a room to hear the effects in, keyed by the same
	# name as its music, so a screen cannot get dungeon music in a menu-sized space. The
	# reverb is what makes this game sound like stone, and it is created in code — so this
	# is the same check the Music bus gets below, asked of the other half of the mix.
	for score in A.SCORES:
		if not A.SPACES.has(score):
			fails += 1; print("FAIL score '%s' has no room in Audio.SPACES" % score)
	for where in A.SPACES:
		if not A.SCORES.has(String(where)):
			fails += 1; print("FAIL Audio.SPACES has a room '%s' nothing plays in" % where)
	if not _source_has("res://scripts/audio.gd", "AudioServer.add_bus_effect(sfx, _reverb)"):
		fails += 1
		print("FAIL nothing puts a reverb on the SFX bus — the effects are dry wherever you are")
