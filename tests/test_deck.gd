...
func _init() -> void:
	...
	Meta_.path_prefix = "t_test_deck_"
	_cleanup_sandbox()   # a flush after a previous run can outlive its cleanup
	Meta_.writes_disabled = false
	load("res://scripts/settings_state.gd").path_override = "user://t_test_deck_settings.json"
	...
	if m.loadout_size({"strike": 99}) != m.owned("strike"):
		fails += 1; print("FAIL clamp size %d (owned %d)" % [m.loadout_size({"strike": 99}), m.owned("strike")])

	# too-small deck is invalid
	if m.deck_valid({"strike": 2}): fails += 1; print("FAIL small deck accepted")

	# build_deck materializes clamped copies at collection level
	var d = m.build_deck({"strike": 2, "defend": 3})
	...
	for i in 4:
		m.add_card("strike")               # strike 4 -> 8, total 12
	m.add_gold(m.fuse_gold_cost("strike"))  # fusion is bought with gold too
	if not m.fuse("strike"):
		fails += 1; print("FAIL fuse rejected with ample copies")
	var owned_after: int = m.owned("strike")
	var d2 = m.build_deck({"strike": 99})  # clamp to what is owned
	...
	# save / delete / persist
	m.save_deck("Aggro", {"strike": 2, "bash": 0, "defend": 6})  # bash not owned -> dropped
	if m.decks["Aggro"].has("bash"): fails += 1; print("FAIL unowned card saved")
	m.save_game()
	...
	if int(m2.decks["Aggro"]["strike"]) != 2: fails += 1; print("FAIL deck content not persisted")
