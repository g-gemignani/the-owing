...
**213 files wanted · 3 already present · 210 to provide.**
...
*4 files, 4 still to provide.* There is no player character on screen anywhere in the game.
...
*35 files, 35 still to provide.* Facing LEFT, lit from the left, transparent, NO baked ground shadow (the backdrop supplies the floor). Filenames are archetype ids: naming them this way lets `PixelArt.OVERRIDES` and its positional-assignment fallback be deleted.
