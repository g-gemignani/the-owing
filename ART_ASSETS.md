...
**211 files wanted · 201 already present · 10 to provide.**
...
*4 files, 1 still to provide.* Twelve screens, four paintings. The grouping is by PLACE and the argument for it is in `META_BG` in the tool; these are the screens that passed nothing to `UI.screen()` and were rendering the procedural tiling pattern (D123).
