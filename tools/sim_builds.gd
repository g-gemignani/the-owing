			...
			match int(node["type"]):
				...
				_:
					...
					if eng.lost():
						...
						_tick("rewards", t_rw)
						if won_card != null and not NO_REWARDS:
...
func _live_value(eng: CombatEngine, c: CardData, want_damage: bool) -> int:
	if not want_damage:
		return eng.card_block(c)
	return eng.card_damage(c) + _poison_value(eng, c)

## What applying this card's Poison is worth, in damage.
##
## THE ONE DRIVER CHANGE IN THIS COPY, and the reason it is here: `sim_balance.gd` values a
## card at `card_damage` or `card_block`, and a card that only applies Poison has neither. Its
## own comment at `Policy` names this — "the poison-only cards it could not value" — and the
## blind spot does not matter while poison is one row of fifteen. It decides the whole answer
## when the question is which BUILD is strongest, because a driver that never plays Venom Fang
## reports the poison deck as the weakest thing in the game.
##
## Poison bites for its full stack at end of turn and then decays by one, so `p` stacks left to
## run deal p + (p-1) + ... = p(p+1)/2. What one card adds is the difference between that sum
## before and after, bounded by a horizon: poison landed on turn 9 of a 10-turn fight does not
## collect. The horizon is a constant rather than an estimate on purpose — an estimator would be
## a second opinion about how long the fight lasts, and this driver is three greedy passes.
const POISON_HORIZON := 5

func _poison_value(eng: CombatEngine, c: CardData) -> int:
	var add := c.eff_poison()
	if add <= 0:
		return 0
	var total := 0
	var targets: Array = []
	if c.aoe:
		...
		if foe != null:
			targets.append(foe)
	for t in targets:
		total += _dot_sum(t.poison + add) - _dot_sum(t.poison)
	return total

## Damage a stack of `p` Poison deals over `POISON_HORIZON` turns.
static func _dot_sum(p: int) -> int:
	if p <= 0:
		return 0
	if p <= POISON_HORIZON:
		return p * (p + 1) / 2
	return POISON_HORIZON * p - POISON_HORIZON * (POISON_HORIZON - 1) / 2
...
# =============================================================================
# Deck comparison harness (a COPY of sim_balance.gd, not a replacement for it).
#
# The question this answers is not the one `sim_balance.gd` answers. That tool asks
# "is the CONTENT priced", and its profiles are instruments chosen to hold one mechanic
# each. This one asks "which DECK is strongest", so every row is a deck a player would
# actually build, every row is measured on the same dungeons at the same clears, and the
# only thing that differs between rows is the twenty cards.
#
# Three deliberate departures from the parent tool, all of them so that the deck is the
# only variable:
#   * reward cards are OFF (`--no-rewards`, default on here). A run that picks up eight
#     cards is measuring the reward table as much as the deck it started with.
#   * every row carries the same power at the same level, so no row wins on its power.
#   * poison is valued by the driver (see `_poison_value`), or the poison rows measure
#     the driver's blind spot instead of the cards.
# =============================================================================

## Reward cards are not added to the run deck. Default ON here; see the block above.
static var NO_REWARDS := true

## Max copies of one card in a built deck. The game has no such limit — you may play as
## many copies as you own — but a deck of twenty Gashes is a report about one card, and
## the levelling economy eats duplicates, so a maxed card is a thing you own few of.
const COPY_CAP := 3
const DECK_SIZE := 20

static var MODE := "builds"
static var SEARCH_ROUNDS := 14
static var SEARCH_CANDIDATES := 14
## Start the climb from this authored build instead of from Hack/Cover. A climb is a local
## search, so where it starts is part of what it measures: two starts that meet in the same
## place is evidence, and one that does not is a second answer worth having.
static var SEED_BUILD := ""
## Restrict the climb's pool to these ids. A climb over a hundred cards spends every round
## sampling swaps that were never going to be kept: the first attempt improved four times in
## forty rounds and spent the other thirty-six proving that a random card is worse than the
## one already in the slot. The league below is what narrows it, and this is where the answer
## goes back in.
static var POOL_OVERRIDE: Array = []
...
func _init() -> void:
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--trials="):
			...
			SEARCH_ROUNDS = maxi(1, int(arg.substr(9)))
		elif arg.begins_with("--candidates="):
			SEARCH_CANDIDATES = maxi(2, int(arg.substr(13)))
		elif arg == "--rewards":
			NO_REWARDS = false
		elif arg.begins_with("--maxed="):
			MAXED_DUNGEONS = Array(arg.substr(8).split(","))
		elif arg.begins_with("--basic="):
			BASIC_DUNGEONS = Array(arg.substr(8).split(","))
		elif arg.begins_with("--seed="):
			SEED_BUILD = arg.substr(7)
		elif arg.begins_with("--pool="):
			POOL_OVERRIDE = Array(arg.substr(7).split(","))
		elif arg.begins_with("--seed-ids="):
			SEED_IDS = Array(arg.substr(11).split(","))
	match MODE:
		"builds":
			_build_table()
		"search-maxed":
...
func _card(id: String) -> CardData:
	if not _card_cache.has(id):
		_card_cache[id] = load(CARD_DIR + id + ".tres") as CardData
	return _card_cache[id]

## `ids` cycled until the deck is DECK_SIZE, honouring COPY_CAP. `level` of -1 means
## "as high as this card's own track goes" — which is what maxed means, and it is per
## card rather than a flat 100 because rarity buys a shorter track (`Balance.max_level`).
func _deck_from(ids: Array, level: int) -> Array[CardData]:
	var out: Array[CardData] = []
	var count := {}
	var i := 0
	var stuck := 0
	while out.size() < DECK_SIZE and stuck < ids.size() * 2:
		var id := String(ids[i % ids.size()])
		i += 1
		if int(count.get(id, 0)) >= COPY_CAP:
			stuck += 1
			continue
		stuck = 0
		count[id] = int(count.get(id, 0)) + 1
		var c := (_card(id)).duplicate() as CardData
		c.level = c.level_cap() if level < 0 else level
		out.append(c)
	return out

func _build_ids(build_id: String) -> Array:
	var b := load("res://resources/builds/" + build_id + ".tres") as BuildData
	return Array(b.cards)

## Every card in the catalogue, and the common-only subset of it. `basic` means COMMON
## rarity: the cards a run is dealt before it has cleared anything.
func _pool(basic: bool) -> Array:
	var out: Array = []
	for f in DirAccess.get_files_at(CARD_DIR):
		...
		if c == null:
			continue
		if basic and c.rarity != CardData.Rarity.COMMON:
			continue
		out.append(id)
	out.sort()
	return out
...
## Overridable, because the band a deck separates in is not known before the first run:
## a cell at 0% or 100% ranks nothing, and which dungeons those are moves with every
## change to the content. `--maxed=` / `--basic=`, comma separated.
static var MAXED_DUNGEONS: Array = ["fungal_deep", "sunken_vault", "drowned_market"]
static var BASIC_DUNGEONS: Array = ["crypt", "warrens", "foundry"]
const MAXED_CLEARS := 10
const BASIC_CLEARS := 1
const POWER_ID := "bulwark"

func _power(level: int) -> PowerData:
	var pd := Balance.power(POWER_ID)
	if pd == null:
		return null
	pd = pd.duplicate()
	pd.level = level
...
## RUN completion per dungeon, plus the fight-level diagnostics. One row of the table.
func _score(deck: Array[CardData], dungeons: Array, clears: int, power_level: int,
		trials: int, want_fights: bool) -> Dictionary:
	var pw := _power(power_level)
	var completes: Array = []
	var hp50: Array = []
	var fights: Array = []
	var tiers: Array = []
	for dungeon_id in dungeons:
		var dd := Balance.dungeon(dungeon_id)
		var roster: Array = Array(dd.enemy_roster) if dd != null and dd.has_roster() else []
		var eff := maxi(clears, Balance.effective_gate(dungeon_id))
		var run := _measure_run(dungeon_id, deck, [], roster, Policy.SMART, trials, eff, pw)
		completes.append(float(run["complete"]))
		hp50.append(float(run["hp_p50"]))
		fights.append(float(run["avg_fights"]))
		if want_fights:
			var row: Array = []
			for tier in [Balance.Tier.NORMAL, Balance.Tier.ELITE, Balance.Tier.BOSS]:
				var r := _measure(deck, dd.difficulty if dd != null else 1, tier, 1.0, [], roster, pw)
				row.append(r)
			tiers.append(row)
	var total := 0.0
	for c in completes:
		total += c
	return {
		"completes": completes, "hp50": hp50, "fights": fights, "tiers": tiers,
		"mean": total / float(maxi(1, completes.size())),
		"ratio": Balance.power_ratio(deck, [], pw),
	}

func _row_line(name: String, s: Dictionary, dungeons: Array) -> String:
	var line := "%-26s ratio %6.2f  mean RUN %3.0f%% |" % [name, s["ratio"], s["mean"] * 100.0]
	for i in dungeons.size():
		line += " %s %3.0f%%(hp%2.0f%%)" % [
			String(dungeons[i]).substr(0, 4), float(s["completes"][i]) * 100.0,
			float(s["hp50"][i]) * 100.0]
	return line

# --- mode: builds ------------------------------------------------------------

func _build_table() -> void:
	var build_ids := ["exhaust", "fortress", "poison", "strength", "swarm", "tempo",
		"thorns", "vampire"]
	print("=== Deck strength, %d trials per cell ===" % TRIALS)
	print(_difficulty_line())
	print("Every row: 20 cards, same power (%s), same clears, no reward cards." % POWER_ID)
	print("Enemies scale to the deck's own `power_ratio`, so a richer deck faces a harder")
	print("dungeon — which is what makes these rows comparable at all.\n")

	print("--- MAXED (every card at its own level cap, power Lv4, %d clears) ---" % MAXED_CLEARS)
	print("    dungeons: %s\n" % ", ".join(MAXED_DUNGEONS))
	var maxed: Array = []
	for b in build_ids:
		var deck := _deck_from(_build_ids(b), -1)
		var s := _score(deck, MAXED_DUNGEONS, MAXED_CLEARS, 4, TRIALS, false)
		maxed.append([b, float(s["mean"]), s])
		print(_row_line(b, s, MAXED_DUNGEONS))
	maxed.sort_custom(func(a, b): return float(a[1]) > float(b[1]))
	print("\n  order: %s\n" % ", ".join(maxed.map(func(r): return "%s %.0f%%" % [r[0], float(r[1]) * 100.0])))

	print("--- BASIC (every card Lv1, power Lv1, %d clear) ---" % BASIC_CLEARS)
	print("    dungeons: %s\n" % ", ".join(BASIC_DUNGEONS))
	var basic: Array = []
	for b in build_ids:
		# COMMON cards only: a build's rares are not in a fresh collection, and a row that
		# holds them is not the player this half of the table is about.
		var ids: Array = []
		for id in _build_ids(b):
			if _card(String(id)).rarity == CardData.Rarity.COMMON:
				ids.append(id)
		if ids.size() < 3:
			print("%-26s (only %d common cards — no common-only version of this build)" % [b, ids.size()])
			continue
		var deck := _deck_from(ids, 1)
		var s := _score(deck, BASIC_DUNGEONS, BASIC_CLEARS, 1, TRIALS, false)
		basic.append([b, float(s["mean"]), s])
		print(_row_line(b + " (commons)", s, BASIC_DUNGEONS))
	basic.sort_custom(func(a, b): return float(a[1]) > float(b[1]))
	print("\n  order: %s" % ", ".join(basic.map(func(r): return "%s %.0f%%" % [r[0], float(r[1]) * 100.0])))

# --- mode: search ------------------------------------------------------------

## Hill-climb on the deck itself: swap ONE card, keep the swap if it measures better.
##
## The incumbent is RE-MEASURED every round rather than remembered. Completion over a few
## hundred trials carries a few points of noise, and a climber that keeps a remembered
## high score climbs the noise instead of the deck — it locks in whichever round got lucky
## and then refuses every real improvement smaller than that luck.
func _search(maxed: bool) -> void:
	var pool: Array = POOL_OVERRIDE if not POOL_OVERRIDE.is_empty() else _pool(not maxed)
	var dungeons: Array = MAXED_DUNGEONS if maxed else BASIC_DUNGEONS
	var clears: int = MAXED_CLEARS if maxed else BASIC_CLEARS
	var plevel := 4 if maxed else 1
	var level := -1 if maxed else 1
	print("=== Deck search (%s), %d trials per cell, %d rounds, %d candidates ===" % [
		"maxed" if maxed else "basic commons", TRIALS, SEARCH_ROUNDS, SEARCH_CANDIDATES])
	print("pool: %d cards | dungeons: %s | clears: %d\n" % [pool.size(), ", ".join(dungeons), clears])
	# Seed: the plainest deck the game hands out, so the climb has to earn every point.
	var ids: Array = []
	if SEED_IDS.size() == DECK_SIZE:
		# Verbatim, copy cap and all: a climbed deck holds one of several cards, and cycling
		# it through `_deck_from` would hand back a different deck than the one being checked.
		ids = SEED_IDS.duplicate()
	elif not SEED_IDS.is_empty():
		ids = _deck_from(SEED_IDS, 1).map(func(c): return c.id)
	elif SEED_BUILD != "":
		for c in _deck_from(_build_ids(SEED_BUILD), 1):
			ids.append(c.id)
	else:
		for i in DECK_SIZE:
			ids.append("hack" if i % 2 == 0 else "cover")
	var best := _ids_score(ids, dungeons, clears, plevel, level)
	print("seed   %.0f%%  %s" % [best * 100.0, _fmt(ids)])
	for round_i in SEARCH_ROUNDS:
		var incumbent := _ids_score(ids, dungeons, clears, plevel, level)
		var best_ids: Array = ids
		var best_val := incumbent
		for c in SEARCH_CANDIDATES:
			var trial: Array = ids.duplicate()
			trial[randi() % trial.size()] = pool[randi() % pool.size()]
			if _copies_over_cap(trial):
				continue
			var v := _ids_score(trial, dungeons, clears, plevel, level)
			if v > best_val:
				best_val = v
				best_ids = trial
		if best_val > incumbent:
			ids = best_ids
			print("r%-2d    %.0f%%  %s" % [round_i + 1, best_val * 100.0, _fmt(ids)])
		else:
			print("r%-2d    no improvement (incumbent re-measured at %.0f%%)" % [
				round_i + 1, incumbent * 100.0])
	var final := _score(_deck_from_ids(ids, level), dungeons, clears, plevel, TRIALS * 2, true)
	print("\nFINAL  %s" % _fmt(ids))
	print(_row_line("best", final, dungeons))
	for i in dungeons.size():
		var t: Array = final["tiers"][i]
		print("   %-16s normal %3.0f%% (%.1ft) | elite %3.0f%% (%.1ft) | boss %3.0f%% (%.1ft)" % [
			dungeons[i],
			float(t[0]["win_rate"]) * 100.0, float(t[0]["avg_turns"]),
			float(t[1]["win_rate"]) * 100.0, float(t[1]["avg_turns"]),
			float(t[2]["win_rate"]) * 100.0, float(t[2]["avg_turns"])])

func _copies_over_cap(ids: Array) -> bool:
	var count := {}
	for id in ids:
		count[id] = int(count.get(id, 0)) + 1
		if int(count[id]) > COPY_CAP:
			return true
	return false

func _deck_from_ids(ids: Array, level: int) -> Array[CardData]:
	var out: Array[CardData] = []
	for id in ids:
		var c := (_card(String(id))).duplicate() as CardData
		c.level = c.level_cap() if level < 0 else level
		out.append(c)
	return out

## The number the climb maximises, and it is NOT completion alone.
##
## Completion is flat at both ends: every deck reads 0% in the Maw until one of them can
## finish it, and every deck reads 100% in the Crypt. A climber on a flat objective takes a
## random walk. So a run that got eight fights deep and died scores above one that died on
## the second, and the term is small enough (0.30 of a whole clear, at ten fights) that it
## can never outrank actually finishing.
const DEPTH_CREDIT := 0.30
const DEPTH_FIGHTS := 10.0

func _ids_score(ids: Array, dungeons: Array, clears: int, plevel: int, level: int) -> float:
	var s := _score(_deck_from_ids(ids, level), dungeons, clears, plevel, TRIALS, false)
	var total := 0.0
	for i in (s["completes"] as Array).size():
		total += float(s["completes"][i]) \
			+ DEPTH_CREDIT * minf(1.0, float(s["fights"][i]) / DEPTH_FIGHTS)
	return total / float(maxi(1, (s["completes"] as Array).size()))

static func _fmt(ids: Array) -> String:
	var count := {}
	for id in ids:
		count[id] = int(count.get(id, 0)) + 1
	...
	var parts: Array = []
	for k in keys:
		parts.append("%dx%s" % [int(count[k]), k])
	return ", ".join(parts)


## Does the driver actually PLAY the deck? (D124's standing question.)
##
## Six turns against a target that cannot die, then the fight tally. A build whose whole
## mechanic reads zero here is a build the report cannot price, and the row it produces is
## a verdict on the driver wearing the cards' clothes.
func _probe() -> void:
	for b in ["poison", "thorns", "strength", "swarm", "vampire", "fortress", "tempo", "exhaust"]:
		var deck := _deck_from(_build_ids(b), -1)
		var eng := CombatEngine.new()
		eng.setup(deck, 1 << 22, 1 << 22, 5, Balance.Tier.NORMAL, "", [], [], _power(4), "", [])
		for i in range(1, eng.enemies.size()):
			eng.enemies[i].hp = 0
		eng.enemies[0].max_hp = 1 << 26
		eng.enemies[0].hp = 1 << 26
		...
		for _t in 6:
			...
			eng.end_turn()
		var t := eng.fight_tally()
		print("%-10s dmg/turn %7.0f | poison applied %5d | block %6d | cards %d in %d turns" % [
			b, float(int(t.get(Balance.TALLY_DAMAGE, 0))) / float(maxi(1, turns)),
			int(t.get(Balance.TALLY_POISON, 0)), int(t.get(Balance.TALLY_BLOCK, 0)),
			int(t.get(Balance.TALLY_CARDS, 0)), turns])


## Explicit starting deck for the climb, cycled to DECK_SIZE like any other id list.
static var SEED_IDS: Array = []

## What is each card WORTH, measured one card at a time.
##
## The climb could not answer this and the reason is worth writing down: a single swap in a
## twenty-card deck moves the run by less than the noise on sixty trials, so forty rounds of
## random swaps improved four times and the other thirty-six measured the weather. A league
## does not have that problem, because it changes a QUARTER of the deck at once — four copies
## against a fixed baseline — and four copies of a good card is a difference no amount of
## noise hides.
##
## What it is NOT: a ranking of cards in the abstract. Four copies of a payoff whose enabler
## is not in the baseline scores as the payoff never firing, which is a true fact about that
## deck and a false one about the card. The climb afterwards is what puts the pairs back
## together; this pass only says which cards are worth climbing over.
func _card_league(maxed: bool) -> void:
	var pool: Array = POOL_OVERRIDE if not POOL_OVERRIDE.is_empty() else _pool(not maxed)
	var dungeons: Array = MAXED_DUNGEONS if maxed else BASIC_DUNGEONS
	var clears: int = MAXED_CLEARS if maxed else BASIC_CLEARS
	var plevel := 4 if maxed else 1
	var level := -1 if maxed else 1
	print("=== Card league (%s), %d trials, dungeons %s ===" % [
		"maxed" if maxed else "basic commons", TRIALS, ", ".join(dungeons)])
	print("Baseline: 8x hack, 8x cover, 4x <card>. Score = mean completion + depth credit.\n")
	var base: Array = []
	for i in 16:
		base.append("hack" if i % 2 == 0 else "cover")
	var baseline := _ids_score(base + ["hack", "cover", "hack", "cover"], dungeons, clears, plevel, level)
	print("baseline (10x hack, 10x cover)  %.1f%%\n" % [baseline * 100.0])
	var rows: Array = []
	for id in pool:
		var ids: Array = base.duplicate()
		for i in 4:
			ids.append(id)
		var v := _ids_score(ids, dungeons, clears, plevel, level)
		rows.append([id, v])
		print("   %-18s %5.1f%%  (%+.1f)" % [id, v * 100.0, (v - baseline) * 100.0])
	rows.sort_custom(func(a, b): return float(a[1]) > float(b[1]))
	print("\n--- ranked ---")
	for r in rows:
		print("%-18s %5.1f%%" % [r[0], float(r[1]) * 100.0])
	print("\ntop20: %s" % ",".join(rows.slice(0, 20).map(func(r): return String(r[0]))))


## One named deck, measured across a ladder of dungeons and reported in full. No search.
##
## The climb reports its own best at the trial count it was climbing on, which is chosen for
## speed rather than for confidence — and a number quoted to a reader has to be the second
## kind. This re-measures a finished deck at whatever `--trials=` is given, on every dungeon
## asked for, with the per-tier fight rates beside the run rate.
func _measure_one(maxed: bool) -> void:
	var ids: Array = SEED_IDS if SEED_IDS.size() == DECK_SIZE \
		else _deck_from(SEED_IDS, 1).map(func(c): return c.id)
	var dungeons: Array = MAXED_DUNGEONS if maxed else BASIC_DUNGEONS
	var clears: int = MAXED_CLEARS if maxed else BASIC_CLEARS
	var deck := _deck_from_ids(ids, -1 if maxed else 1)
	var s := _score(deck, dungeons, clears, 4 if maxed else 1, TRIALS, true)
	print("=== %s deck, %d trials ===" % ["maxed" if maxed else "basic (Lv1 commons)", TRIALS])
	print("%s\n" % _fmt(ids))
	print(_row_line("deck", s, dungeons))
	for i in dungeons.size():
		var t: Array = s["tiers"][i]
		print("   %-16s RUN %3.0f%% | normal %3.0f%% (%.1ft, -%.0f%%hp) | elite %3.0f%% (%.1ft, -%.0f%%hp) | boss %3.0f%% (%.1ft, -%.0f%%hp)" % [
			dungeons[i], float(s["completes"][i]) * 100.0,
			float(t[0]["win_rate"]) * 100.0, float(t[0]["avg_turns"]), float(t[0]["hp_lost_pct"]) * 100.0,
			float(t[1]["win_rate"]) * 100.0, float(t[1]["avg_turns"]), float(t[1]["hp_lost_pct"]) * 100.0,
			float(t[2]["win_rate"]) * 100.0, float(t[2]["avg_turns"]), float(t[2]["hp_lost_pct"]) * 100.0])
